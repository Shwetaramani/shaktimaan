...
import { AddMatchesComponent } from "src/app/@shared/components/add-matches/add-matches.component";
declare var $: any;
...
export class MatchComponent implements OnInit {
  ...
  // showTemp: any = localStorage.getItem('addBanner')
  items: any = [];
  slides: any = [
  ...
  addedItems: any = [true, false];
  matchblocks: any = [];
  showInsight: boolean = false;
  @ViewChild("videoElement") videoElement!: ElementRef;
  constructor(private router: Router, private _snackBar: MatSnackBar, private modalService: NgbModal) {
  ...
  deleteCard(i: number = 0): void {
    this.items[i] = 'delete';
    this.openSnackBar('Match card deleted', 'close');
  }
  ...
  openMachesModal(): void {
    const modalRef = this.modalService.open(AddMatchesComponent, { modalDialogClass: 'sm-modal', size: 'lg' });
    modalRef.result.then((result) => {
      console.log(result);
      if (result.addMatch == true) {
        this.addCard = true;
        this.matchblocks.push(true);
      }
    }, (reason) => {
    });
